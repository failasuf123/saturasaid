...
import { WeddingBannerList } from '@/shared/WeddingBannerListLandingPage';
...
function TemplateWeddingListLandingPage() {
    const data = WeddingBannerList;

    return (
        <div className="w-full gap-3 py-8 md:gap-5  grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4">
            {data.map((item) => (
                
                <Link key={item.id} href={`/create/${item.theme}`}>
                    <motion.div
                        className="bg-white p-6 rounded-lg shadow-lg"
                        whileHover={{ scale: 1.05 }}
                        initial={{ opacity: 0, y: 100 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        transition={{ duration: item.id, delay: 0.3 }}
                    >
...
export default TemplateWeddingListLandingPage;